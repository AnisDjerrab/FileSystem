/* This code is responsible of Creating Files In the file system
 * The creation of files is done in 5 essential steps : creation of it's header (name, type...), creation of it's content, of it's local inode table (that contains informations about the Blocks's index), And Finally adding it to The Inode & Indexation tables
 ...
 * this may be the most essential function in all this program.
*/
...
unique_ptr<WriteFile> CreateStruct(unique_ptr<char[]> p, unique_ptr<char[]> c, unique_ptr<char[]> t, unique_ptr<char[]> n, unique_ptr<char[]> d, string PathOfTheFS, char permissions) __asm__("CreateStruct");
unique_ptr<WriteFile> CreateStruct(unique_ptr<char[]> p, unique_ptr<char[]> c, unique_ptr<char[]> t, unique_ptr<char[]> n, unique_ptr<char[]> d, string PathOfTheFS, char permissions) {
    auto W = make_unique<WriteFile>();
    W->Path = move(p);
    W->content = move(c);
    W->type = move(t);
    W->name = move(n);
...
__attribute__((visibility("default")))
void CreateFile(bool UserJournalisation, bool Journalisation, WriteFile* W, const char* RealNameOfTheFile, unordered_map<int, int>& freeBlocks, int& SizeOfTheFile, unordered_map<int, int>& disponibleSpotsInTheIndexationTable) __asm__("CreateFile");
void CreateFile(bool UserJournalisation, bool Journalisation, WriteFile* W, const char* RealNameOfTheFile, unordered_map<int, int>& freeBlocks, int& SizeOfTheFile, unordered_map<int, int>& disponibleSpotsInTheIndexationTable) {
    vector<int> indexes;
    ...
    contentOfTheHeader->append(W->type.get());
    ...
    //creation of The Content Blocks
    UsedFreeBlocks = IndexesOfTheBlocksOfTheHeader->size();
    IndexInTheData.insert(IndexInTheData.begin(), IndexesOfTheBlocksOfTheHeader->begin(), IndexesOfTheBlocksOfTheHeader->end());
    IndexesOfTheBlocksOfTheHeader.reset();
    IndexesOfTheIdentifiersOfTheHeader.reset();
    lengthOfTheHeader.reset();
    auto IndexOfTheBlocksOfTheMainContent = make_unique<vector<int>>();
    if (strlen(W->content.get()) != 0) {
        auto lengthOfTheMainContent = make_unique<int>(strlen(W->content.get()) + 1);
        auto IndexOfTheIdentifiersOfTheMainContent= make_unique<vector<int>>();
        for (size_t i = 0; ;i++) {
            if (i == 0) {
                *lengthOfTheMainContent += 8;
                BlocksToAllocate += 1;
            } else {
                BlocksToAllocate += 1;
            }
            if (*lengthOfTheMainContent < BlocksToAllocate * SizeOfTheBlock) {
                break;
            }
        }
        ...
        IndexOfTheBlocksOfTheMainContent->insert(IndexOfTheBlocksOfTheMainContent->end(), temp->begin(), temp->end());
        temp.reset();
        for (int i = 0; i < IndexOfTheBlocksOfTheMainContent->size(); i++) {
            if (i == 0) {
                IndexOfTheIdentifiersOfTheMainContent->push_back(i);
            } else {
                if (IndexOfTheBlocksOfTheMainContent->at(i - 1) + SizeOfTheBlock != IndexOfTheBlocksOfTheMainContent->at(i)) {
                    IndexOfTheIdentifiersOfTheMainContent->push_back(i);
                    *lengthOfTheMainContent += 8;
                }
            }
            if (*lengthOfTheMainContent > IndexOfTheBlocksOfTheMainContent->size() * SizeOfTheBlock) {
                IndexOfTheBlocksOfTheMainContent->push_back(Dynamic_Functions::listFreeBlocks(Journalisation, UserJournalisation, freeBlocks, freeSegments, disponibleSpotsInTheIndexationTable, indexes, SizeOfTheFile, Write, 1)[0]);
            }
        }
        BlocksToAllocate = 0;
        IndexAchieved = 0;
        ...
        while (true) {
            bool exit = false;
            bool found = false;
            for (size_t o = 0; o < IndexOfTheIdentifiersOfTheMainContent->size(); o++) {
                if (IndexAchieved == IndexOfTheIdentifiersOfTheMainContent->at(o)) {
                    found = true;
                    break;
                }
            }
            auto content = make_unique<char[]>(SizeOfTheBlock);
            if (found) {
                if (strlen(W->content.get()) + 1 - IndexInTheContent < SizeOfTheBlock - 8) {
                    memcpy(content.get(), identifier.get(), 8);
                    memcpy(content.get() + 8, W->content.get(), strlen(W->content.get()) + 1 - IndexInTheContent);
                    exit = true;
                } else {
                    memcpy(content.get(), identifier.get(), 8);
                    memcpy(content.get() + 8, W->content.get() + IndexInTheContent, SizeOfTheBlock - 8);
                    IndexInTheContent += SizeOfTheBlock - 8;
                }
            } else {
                if (strlen(W->content.get()) + 1 - IndexInTheContent < SizeOfTheBlock) {
                    memcpy(content.get(), W->content.get() + IndexInTheContent, strlen(W->content.get()) + 1 - IndexInTheContent);
                    exit = true;
                } else {
                    memcpy(content.get(), W->content.get() + IndexInTheContent, SizeOfTheBlock);
                    IndexInTheContent += SizeOfTheBlock;
                }
            } 
            Write.seekp(IndexOfTheBlocksOfTheMainContent->at(IndexAchieved), ios::beg);
            Write.write(content.get(), SizeOfTheBlock);
            content.reset();
            IndexAchieved++;
            if (exit) {
    ...
    LogJournalWriter << "Succesfully writed the data of the file : " << RealNameOfTheFile << " using : " << IndexOfTheBlocksOfTheMainContent->size() << " Blocks." << endl;
    IndexInTheData.insert(IndexInTheData.begin(), IndexOfTheBlocksOfTheMainContent->begin(), IndexOfTheBlocksOfTheMainContent->end());
    ...
    MappageOfAllTheClustersAndInodes->push_back(move(Dynamic_Functions::ConvertNumberInto64BitsString(-900)));
    IndexAchieved = 0;
    for (size_t i = 0; i < UsedFreeBlocks - 1; i++) {
        ...
        if (FirstValue != secondValue) {
            MappageOfAllTheClustersAndInodes->push_back(move(Dynamic_Functions::ConvertNumberInto64BitsString(-800)));
    ...
    UsedFreeBlocks += IndexOfTheBlocksOfTheMainContent->size();
    IndexOfTheBlocksOfTheMainContent.reset();
    MappageOfAllTheClustersAndInodes->push_back(Dynamic_Functions::ConvertNumberInto64BitsString(-700));
    for (size_t i = IndexAchieved; i < UsedFreeBlocks - 1; i++) {
        auto FirstValue = Dynamic_Functions::ConvertNumberInto64BitsString(IndexInTheData[i]);
        while(IndexInTheData[i] + SizeOfTheBlock == IndexInTheData[i + 1]) {
            if (!(i + 2 < UsedFreeBlocks - 1)) {
                break;
        ...
        auto secondValue = Dynamic_Functions::ConvertNumberInto64BitsString(IndexInTheData[i + 1]);
        if (FirstValue != secondValue) {
            MappageOfAllTheClustersAndInodes->push_back(Dynamic_Functions::ConvertNumberInto64BitsString(-600));
            MappageOfAllTheClustersAndInodes->push_back(move(FirstValue));
            MappageOfAllTheClustersAndInodes->push_back(move(secondValue));
        } else {
            MappageOfAllTheClustersAndInodes->push_back(move(FirstValue));
    ...
    ContentOfTheInodeBlock->append(W->type.get(), 2);
    ContentOfTheInodeBlock->append(Dynamic_Functions::ConvertNumberInto64BitsString(0).get(), 8); //size of the file
    ContentOfTheInodeBlock->append(Dynamic_Functions::ConvertNumberInto64BitsString(strlen(W->content.get()) + 1).get(), 8);
...
__attribute__((visibility("default")))
void DestroyStruct(unique_ptr<WriteFile> W) __asm__("DestroyStruct");
void DestroyStruct(unique_ptr<WriteFile> W) {
    W.reset();
}
